...
api.post("/addBooking", async (req, res) => {
  ...
  try {
    ...
    const { rows: existing } = await db.query(
      `SELECT * FROM bookings WHERE "bookingDate"='${bookingDate}' AND "location"='${location}'`
    );
    ...
    await db.query(
      `INSERT INTO bookings ("id", "createdDate", "bookingDate", "location", "username") VALUES ('${id}', NOW(), '${bookingDate}', '${location}', '${username}')`
    );
...
// DELETE a booking
api.delete("/deleteBooking", async (req, res) => {
  const db = await connect();
  // Catch errors
  try {
